...
const Judge = ({details}) => {
    ...
    const [dateAvailable, setDateAvailable] = useState(['2021-09-21 - 2021-09-22', '2021-09-23 - 2021-09-24', '2021-09-25 - 2021-09-26']);
  return (
    <div className={!isExpanded ? "judge" : "judge active"}>
        <div className='judgeTop' onClick={()=>{setIsExpanded(!isExpanded)}}>
            <div className='judgeImage'>
            ...
            <div className='judgeDetails'>
                <span className='judgeName'>{details.judgeName}</span>
                <div>
                    <span className='judgeType'>{details.specialization} - </span>
                    <span className='judgeCourt'>{details.court} - </span>
                    <span className='judgeExp'>
                        <span>{details.yearsOfExperience} years experience</span>
                    </span>
                </div>
            </div>
        ...
        {isExpanded && 
            <div className='judgeBottom'>
                <div className='dateAvailable'>
                    <div className='title'>Date Available:</div>
                    <div className='value'>
                        {dateAvailable.map((date, index) => {
                            return (
                                <div className='date' key={index}> 
                                    <input type="checkbox" id={date} name={date} value={date} /> {date}</div>
                            )
                        })}
                    </div>        
                </div>
